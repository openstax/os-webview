...
function useContextValue<T extends {value: string; text: string}>({
    maxSelections
}: {
    maxSelections: number;
}) {
    const [data, dispatch] = useReducer(
        (state: T[], [action, item]: [string, T]) => {
            const itemsOtherThanItem = state.filter((i) => i !== item);
            const canAdd =
                !maxSelections || itemsOtherThanItem.length < maxSelections;

            switch (action) {
                case 'add':
                    return canAdd ? [...itemsOtherThanItem, item] : state;
                // this is the 'remove' case, but also the only other possibility
                default:
                    return itemsOtherThanItem;
            }
        },
        []
    );
...
export {useContext as default, ContextProvider as MultiselectContextProvider};
