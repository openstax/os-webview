    ...
    function cnxFetch() {
        ...
        if (isRex) {
            return fetch(`${rexOrigin}/rex/environment.json`)
                ...
                .then((r) => r.json())
                .then((r) => {
                    console.info('Fetched info', {
                        cnxId,
                        bookThing: r.books[cnxId],
                        fetchAddr: `//archive.cnx.org/contents/${cnxId}@${r.books[cnxId].defaultVersion}`
                    });
                    return fetch(`//archive.cnx.org/contents/${cnxId}@${r.books[cnxId].defaultVersion}`);
                })
                .then((r) => r.json());
        }
