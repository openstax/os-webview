...
const resourcePromise = getFields('resources').then(
    (resources: {field: string}[]) => resources.map((entry) => entry.field)
);
...
const subnotes: Record<string, string> = {
    Textbook: 'includes print, PDF, and web view'
};
...
function useSourceTypes() {
    ...
    const [selectedSource, updateSelectedSource] = useState<string | null>(
        null
    );
...
export default function ErrorSourceSelector() {
    ...
    const [RadioInvalidMessage, updateRadioInvalidMessage] =
        managedInvalidMessage(radioRef);

    React.useEffect(updateRadioInvalidMessage, [
        sourceTypes,
        selectedSource,
            ...
            <fieldset className="radio-columns">
                ...
                {sourceTypes.map((sType, index) => (
                    <LabeledButton
                        ...
                        onChange={onChange}
                        radioRef={index === 0 ? radioRef : undefined}
                    />
                ))}
            </fieldset>
        </React.Fragment>
