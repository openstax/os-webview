...
jest.mock('react-aria-carousel', () => ({
    Carousel: jest.fn().mockImplementation(({children}) => <div className="Carousel">{children}</div>)
}));
...
describe('partner-details', () => {
    ...
    it('shows no synopsis if there is no partnerName', () => {
        ...
        render(<PartnerDetails detailData={data} title="" setTitle={setTitle} />);
    ...
    it('displays images/videos if present', async () => {
        const data = {...partnerData, ...linkTexts, videos: ['/path/to/video'], images: ['/path/to/image']};
        const setTitle = jest.fn();

        render(<PartnerDetails detailData={data} title="" setTitle={setTitle} />);
        await waitFor(() => expect(document.querySelector('.Carousel')).toBeTruthy());
    });
});
