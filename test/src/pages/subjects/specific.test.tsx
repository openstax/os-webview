...
import {mathSubjectContext, tutorAd, infoBoxes, aboutOs} from '../../data/specific-subject';
...
jest.mock('~/pages/subjects/new/specific/context', () => jest.fn(() => ({categories: []})));
...
describe('specific subject page', () => {
    it('renders something', () => {
        render(<Component />);
        expect(screen.getAllByText('Open textbooks')).toHaveLength(1);
    ...
    it('does translations', () => {
        (LazyLoad as jest.Mock).mockImplementation(({children}) => (<React.Fragment>{children}</React.Fragment>)
        );
        (useSpecificSubjectContext as jest.Mock).mockReturnValue(mathSubjectContext);
        render(<Component />);
        expect(screen.getAllByText('Open textbooks')).toHaveLength(1);
    });
    it('does infoboxes and tutor ad when present', () => {
        (LazyLoad as jest.Mock).mockImplementation(({children}) => (<React.Fragment>{children}</React.Fragment>)
        );
        (useSpecificSubjectContext as jest.Mock).mockReturnValue(
            {...mathSubjectContext, tutorAd, infoBoxes, aboutOs}
        );
        render(<Component />);
        expect(screen.getAllByText('Expert authors')).toHaveLength(1);
        expect(screen.getAllByText('Ad heading')).toHaveLength(1);
    });
});
